...
## (5 per age survived by a development, +10 per adapted development).
...
func run(t: TestContext) -> void:
	...
	t.label("heritage axis: survivors and adapted developments")
	...
	t.eq(result3.get("heritage"), 5 * 3 + 10 * 1, "5 per age survived + 10 per adapted")
